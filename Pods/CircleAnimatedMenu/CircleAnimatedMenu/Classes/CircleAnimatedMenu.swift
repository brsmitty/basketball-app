...
@IBDesignable open class CircleAnimatedMenu: UIControl {
    ...
    var startAngle:CGFloat = CGFloat(Double.pi/8)
    var endAngle:CGFloat = CGFloat(Double.pi/8)
    ...
    func updateUI() {
        ...
        for (index, value) in tuplesArray.enumerated() {
            ...
            path.move(to: center)
            endAngle = startAngle + width * CGFloat(Double.pi) * 2.0
            ...
            let middleEndAngle:CGFloat = startAngle + width / 2 * CGFloat(Double.pi) * 2.0
        ...
        textLayer.font = CTFontCreateWithName(titleFont.fontName as CFString? ?? "" as CFString, titleFont.pointSize,  nil)
